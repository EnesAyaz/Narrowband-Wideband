...
float delay_current_control=1000;
...
//    fEpwm2DutyCycle=fEpwm2DutyCycle+0.01;
  //  fEpwm4DutyCycle=fEpwm4DutyCycle+0.01;
   ...
   //     EPwm1Regs.TBPHS.bit.TBPHS=  ((phase+phasePI+Phase_TD))*EPwm1Regs.TBPRD/180; // EPwm2Regs.TBPRD = 180 derece
        EPwm2Regs.TBPHS.bit.TBPHS=  ((Phase_TD))*EPwm2Regs.TBPRD/180; // EPwm2Regs.TBPRD = 180 derece
   //     if (XMODE==0)
     //   {EPwm2Regs.TBPHS.bit.TBPHS=  ((Phase_TD))*EPwm2Regs.TBPRD/180;} // EPwm2Regs.TBPRD = 180 derece
      //  else if (XMODE==1)
  ...
  /*    if ( Curr4> maxCurr || Curr4<-maxCurr)
          {GpioDataRegs.GPDCLEAR.bit.GPIO125 = 1; } // Set 1 (DISABLE)*/
       ...
       else if (Current_control==1)
       {
    	   operation_frequeuency=61000;

    	   if (maq>maq_current_control && iSinusoidalPWMCounter==delay_current_control )
    	                 	    	    		{
    	                 	  maq=maq-0.01;
    	                 	  iSinusoidalPWMCounter=0;
    	                 	    	    		}
