...
#if CONTROLMODE == 1
        /* Open loop Control - Sinusoidal PWM*/
        fEpwm1DutyCycle = duty_cycle;
        fEpwm2DutyCycle = duty_cycle;
        fEpwm4DutyCycle = duty_cycle;
...
EDIS; */
	...
	//    EPwm2Regs.AQCTLA.bit.CAU = 2; //set low
	  ...
	  //  EPwm2Regs.TBPHS.bit.TBPHS=  2*EPwm2Regs.TBPRD/9; // EPwm2Regs.TBPRD = 180 derece
	    EPwm2Regs.TBPHS.bit.TBPHS=  EPwm2Regs.TBPRD; // EPwm2Regs.TBPRD = 180 derece
	    EPwm2Regs.TBCTL.bit.SWFSYNC = 1;
...
void EPWM4Setup(void)
{
	EALLOW;
		    ...
		    //EPwm4Regs.TBPHS.bit.TBPHS= 2*EPwm2Regs.TBPRD/3;
