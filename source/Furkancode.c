...
volatile float duty_op = 0.5; // for DC voltage is required
...
int main(void)
{
   ...
   // ConfigCpuTimer(&CpuTimer0, 200, timer0set ); //100 useconds
    ...
    while(1)
    {
    	...
    	// if (operation_frequeuency>40000 && operation_frequeuency<100000 && duty_cycle>0.1 && duty_cycle<0.9)
    	//	{EPwm1Regs.TBPRD = (pwmclk_frequency) / (operation_frequeuency) /2;
    	//	EPwm2Regs.TBPRD = (pwmclk_frequency) / (operation_frequeuency) /2;
    	//	EPwm1Regs.CMPA.bit.CMPA = round(EPwm1Regs.TBPRD*(1.0-duty_cycle) + -1*dead_time / pwmclk_period /2);
    	//	EPwm2Regs.CMPA.bit.CMPA = round(EPwm2Regs.TBPRD*(1.0-duty_cycle) + -1*dead_time / pwmclk_period /2);
...
__interrupt void cpu_timer0_isr(void)
{
    if (CodeTrig == 1)
    {
     ...
     if (operation_frequeuency>40000 && operation_frequeuency<120000 && duty_cycle>0.01 && duty_cycle<0.99)
        { 		EPwm1Regs.TBPRD = (pwmclk_frequency) / (operation_frequeuency) /2;
        		EPwm2Regs.TBPRD = (pwmclk_frequency) / (operation_frequeuency) /2;
        		EPwm1Regs.CMPA.bit.CMPA = round(EPwm1Regs.TBPRD*duty_cycle);
        		EPwm2Regs.CMPA.bit.CMPA = round(EPwm2Regs.TBPRD*duty_cycle);
        		EPwm2Regs.TBPHS.bit.TBPHS=EPwm2Regs.TBPRD/4;
         }
