...
int main(void)
{
    ...
    while (1)
    {
    	...
    	if (frequency_control<0 )
    		{frequency_control=0;}
    	else if (frequency_control>19)
    	     {frequency_control=19;}


    	/* fModulationIndexDefault=frequency_control/20; */

       /* fundamental_freq=frequency_control;*/
...
__interrupt void cpu_timer0_isr(void)
{
    ...
    if (operation_frequeuency>70000 && operation_frequeuency<90000)
            { 		EPwm1Regs.TBPRD = (SYSTEMCLOCKFREQUENCY) / (operation_frequeuency) /2;
            	    ...
            	    EPwm2Regs.TBPHS.bit.TBPHS=  EPwm2Regs.TBPRD*phase/180; // EPwm2Regs.TBPRD = 180 derece
