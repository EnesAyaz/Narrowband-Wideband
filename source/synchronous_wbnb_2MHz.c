...
#define SWITCHINGFREQUENCY      2000000       // 2 MHz


float operation_frequency = 2000000;  // 2 MHz

#define LOOPFREQUENCY    2000000;  // 2 MHz
...

{
    ...
    InitCpuTimers();
    ConfigCpuTimer(&CpuTimer0, 200, 0.5 ); // 2 MHz (1000000/LOOPFREQUENCY)
...
//#if CONTROLMODE == 1
	...
	if (iSinusoidalPWMCounter == 20){ //((LOOPFREQUENCY / fundamental_freq) )) 50
	iSinusoidalPWMCounter = 0;}
...
//    	        fModulationIndexModule4 = fModulationIndexDefault;
    	        fEpwm1DutyCycle = fModulationIndexDefault*(__sinpuf32(iSinusoidalPWMCounter*0.05)+1)/2; // fundamental_freq*iSinusoidalPWMCounter/ LOOPFREQUENCY
    	        fEpwm2DutyCycle = fModulationIndexDefault*(__sinpuf32(iSinusoidalPWMCounter*0.05-phase_ref)+1)/2;//(__sinpuf32(iSinusoidalPWMCounter*0.05 - phase_ref ) + 1) / 2;
